...
export class HomePage extends React.Component {
      ...
      <div className="invoice-generator">
        ...
        <div className="product-inputs">
          ...
          <div className="buttons-container">
            <button id="addProduct">Add Another Product</button>
        ...
        <div className="print-download-buttons hidden">
          <button id="printInvoice" onClick={this.printInvoice}>Print Invoice</button>
          <button id="printInvoicePDF" onClick={this.printInvoicePDF}>Print Invoice as PDF</button>
        </div>
      </div>
    );
  }
  ...
  // Event listener for showing the invoice preview when "Add Item" is clicked
  ...
  printInvoice(){
    ...
    const invoicePreview = document
      .getElementById("invoicePreview")
      .cloneNode(true); // Clone the preview
    const printWindow = window.open("", "", "width=800,height=600");
    printWindow.document.open();
    printWindow.document.write(
      '<html><head><title>Invoice</title><style>table { border-collapse: collapse; width: 100%; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }</style></head><body style="font-family: Arial, sans-serif;">'
    ); // Apply a font-family
  ...
  printInvoicePDF(){
    ...
    // Define the special element for rendering
    ...
    // Set the title for the PDF (based on the title in your HTML)
    pdf.setProperties({
      title: "Invoice",
    });
    ...
    const tableElement = document.getElementById("invoiceItemsPreview");
    pdf.autoTable({
      html: tableElement,
      startY: 130,
      theme: "grid",
      margin: { left: 15 },
      columnStyles: {
        0: { fontStyle: "bold" }, // Make the first column (SL) bold
        3: { halign: "right" }, // Right-align Unit Price
